...
//   Typography
// } from 'antd'
// import { useMemo, useCallback, useContext, useEffect, useReducer, useState, useRef, Fragment } from 'react'
// import {
//   CreateServicesCardRequestBody,
//   GetServicesCardSoldOfCustomer,
//   ServicesOfCardType
...
// // Reducer để quản lý state liên quan đến thẻ dịch vụ
// const serviceCardReducer = (
//   state,
//   action
// ): {
//   soldCards: CardOfServicesCardSoldOfCustomer[]
//   availableCards: ServicesOfCardType[]
//   selectedCardIds: string[]
//   totalPrice: number
//   newCardData?: ServicesOfCardType
// } => {
//   switch (action.type) {
//     case 'SET_SOLD_CARDS':
//       return { ...state, soldCards: action.payload }
//     case 'SET_AVAILABLE_CARDS':
//       return { ...state, availableCards: action.payload }
//     case 'SET_SELECTED_CARDS':
//       return { ...state, selectedCardIds: action.payload.ids, totalPrice: action.payload.price }
//     case 'RESET_SELECTED_CARDS':
//       return { ...state, selectedCardIds: [], totalPrice: 0 }
//     case 'SET_NEW_CARD':
//       return { ...state, newCardData: action.payload }
//     case 'REMOVE_NEW_CARD':
//       return { ...state, newCardData: undefined }
//     default:
//       return state
//   }
// }
...
//   const [serviceCardState, dispatch] = useReducer(serviceCardReducer, {
//     soldCards: [],
//     availableCards: [],
//     selectedCardIds: [],
//     totalPrice: 0,
...
//   // Cập nhật thẻ dịch vụ đã bán khi data thay đổi
//   useEffect(() => {
//     if (servicesCardSoldOfCustomerData) {
//       dispatch({ type: 'SET_SOLD_CARDS', payload: servicesCardSoldOfCustomerData.cards })
//     }
//   }, [servicesCardSoldOfCustomerData])

//   // Reset về tab mặc định mỗi khi mở modal
//   useEffect(() => {
//     if (open === StatusOpenModalServicesCard.UPDATE) {
//       setActiveTabKey('123')
//     }
...
//   // Cleanup function for any timeouts when component unmounts
//   useEffect(() => {
//     return () => {
...
//   // Memoize available service cards list
//   const availableServiceCards = useMemo(() => {
//     if (!data) return []

//     const mainListServicesCard = data.data.result.servicesCard
//     // Tạo một Map các ID đã bán để tìm kiếm hiệu quả hơn
//     const soldCardIds = new Set(serviceCardState.soldCards.map((card) => card._id))

//     const filteredCards = mainListServicesCard.filter((card) => !soldCardIds.has(card._id))
//     return filteredCards.slice(0, MAX_PREVIEW)
//   }, [data, serviceCardState.soldCards])

//   // Cập nhật danh sách thẻ có sẵn khi thay đổi
//   useEffect(() => {
//     if (availableServiceCards.length > 0) {
//       dispatch({ type: 'SET_AVAILABLE_CARDS', payload: availableServiceCards })
//     }
//   }, [availableServiceCards])

//   // Xử lý chọn thẻ dịch vụ
//   const handleSelectServiceCard = useCallback((serviceCardIds, totalPrice) => {
//     dispatch({
//       type: 'SET_SELECTED_CARDS',
//       payload: { ids: serviceCardIds, price: totalPrice }
//     })
//   }, [])
...
//       clearTimeout(searchTimeoutRef.current)
//     }
//     searchTimeoutRef.current = setTimeout(() => setResetValueSearchServiceCard(false), 200)
//   }, [])
...
//       // Lưu thẻ dịch vụ mới vào state
//       if (response?.data?.result?.[0]) {
//         dispatch({ type: 'SET_NEW_CARD', payload: response.data.result[0] })
//       }
//     },
//     onError: (error: Error, _, context) => {
//       queryClient.setQueryData(['getAllServicesCard'], context?.previousData)
...
//   // Hàm xử lí việc UPDATE Service card sold of customer
//   const handleUpdateServicesCardSoldOfCustomer = useCallback(() => {
//     try {
//       if (!servicesCardSoldOfCustomerData?._id) {
//         message.error('Dữ liệu không hợp lệ: thiếu ID của thẻ dịch vụ.')
//         return
...
//       const updateData = {
//         _id: servicesCardSoldOfCustomerData._id,
//         card_services_sold_id: serviceCardState.selectedCardIds
//       }

//       updateServicesCardSoldOfCustomer(updateData)
//     } catch (error) {
//       message.error('Lỗi trong quá trình thêm mới thẻ dịch vụ!')
//     }
//   }, [servicesCardSoldOfCustomerData, serviceCardState.selectedCardIds, updateServicesCardSoldOfCustomer])

//   // Hàm xử lý thêm mới thẻ dịch vụ
//   const handleCreateServiceCard = useCallback(
//     async (value: CreateServicesCardRequestBody) => {
//       if (!profile?._id) {
//         message.error('Không thể tạo thẻ dịch vụ: User ID không hợp lệ!')
//         return
//       }
...
//       }
//     },
//     [profile, getBranchList, createServiceCard]
//   )

//   // Hàm xử lí việc UPDATE thẻ được tạo riêng vào Service Card Sold Of Customer
//   const handleUpdateServiceCardSoldOfCutomerCreated = useCallback(() => {
//     try {
//       if (!servicesCardSoldOfCustomerData?._id || !serviceCardState.newCardData?._id) {
//         message.error('Dữ liệu không hợp lệ: thiếu ID của thẻ!')
//         return
//       }
...
//       const updateData = {
//         _id: servicesCardSoldOfCustomerData._id,
//         card_services_sold_id: [serviceCardState.newCardData._id]
//       }
//       updateServicesCardSoldOfCustomer(updateData)
//     } catch (error) {
//       message.error('Lỗi trong quá trình thêm mới thẻ dịch vụ!')
//     }
//   }, [servicesCardSoldOfCustomerData, serviceCardState.newCardData, updateServicesCardSoldOfCustomer])

//   // Hàm xoá Service card vừa mới tạo
//   const handleDeleteServiceCardCreated = useCallback(() => {
//     dispatch({ type: 'REMOVE_NEW_CARD' })
//   }, [])

//   // onFinish Form
//   const onFinish = useCallback(
//     (value: CreateServicesCardRequestBody & { _id?: string }) => {
//       try {
//         const branch = getBranchList(value.branch || [])
//         const serviceData = { ...value, branch }
//         handleCreateServiceCard(serviceData)
...
//       }
//     },
//     [getBranchList, handleCreateServiceCard]
//   )

//   // Check if data fetching was successful
//   const isDataSuccess = data?.data?.success
...
//                   flexWrap: 'wrap',
//                   gap: 24,
//                   justifyContent: serviceCardState.soldCards.length <= 2 ? 'flex-start' : 'flex-start',
//                   alignItems: 'stretch'
//                 }}
//               >
//                 {serviceCardState.soldCards.map((card) => (
//                   <Card
//                     key={card._id}
//                     hoverable
//                     style={{
...
//                         {isLoading ? (
//                           <Skeleton active />
//                         ) : serviceCardState.availableCards.length === 0 ? (
//                           <Empty
//                             image='https://gw.alipayobjects.com/zos/antfincdn/ZHrcdLPrvN/empty.svg'
//                             styles={{ image: { width: '100%', height: '100px', margin: '50px 0' } }}
//                             description={
...
//                             columnsGird={8}
//                             onServiceClick={handleSelectServiceCard}
//                             cards={serviceCardState.availableCards}
//                             customCss={true}
//                             resetCard={resetServicesCardSelected}
//                           />
//                         ) : (
...
//                       </Col>
//                       <Row justify={'end'} style={{ marginTop: '30px' }}>
//                         {serviceCardState.selectedCardIds.length !== 0 && (
//                           <Fragment>
//                             <Tag
//                               icon={<CheckCircleOutlined />}
//                               color='success'
//                               style={{ padding: '5px 10px 5px 10px' }}
//                             >
//                               Tổng thẻ dịch vụ: x{serviceCardState.selectedCardIds.length}
//                             </Tag>
//                             <Tag icon={<DollarOutlined />} color='magenta' style={{ padding: '5px 10px 5px 10px' }}>
//                               {serviceCardState.totalPrice.toLocaleString('vi-VN')} VNĐ
//                             </Tag>
//                           </Fragment>
//                         )}
//                         <Col>
...
//                           >
//                             <Button
//                               disabled={serviceCardState.selectedCardIds.length < 1}
//                               title='Thêm thẻ dịch vụ mới'
//                               type='primary'
//                               style={{ width: 'auto' }}
//                             >
...
//                             </Col>
//                             <Col span={8}>
//                               <Form.Item<CreateServicesCardRequestBody> name='branch' label='Chi nhánh'>
//                                 <OptionsBranch
//                                   placeholder={'Chọn chi nhánh'}
//                                   mode={'multiple'}
//                                   search
...
//                                           </Col>
//                                           <Col span={6}>
//                                             <Form.Item {...restField} name={[name, 'discount']} label='Giảm giá'>
//                                               <InputNumber
//                                                 suffix='đ'
//                                                 style={{ width: '100%' }}
//                                                 formatter={(value) => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
...
//                             <Col>
//                               <PreviewServiceCardCreated
//                                 serviceCardCreatedData={serviceCardState.newCardData as ServicesOfCardType}
//                               />
//                             </Col>
//                           </Row>
//                         )}
...
//                                 <div>
//                                   <Tag bordered={false} color='error'>
//                                     {serviceCardState.newCardData?.name}
//                                   </Tag>
//                                   mới tạo không?
//                                 </div>
//                               </Typography>
